...
# ── 유권해석 교차탐지 프롬프트 (refresh_golden_set.py 전용) ─────────────────
# 목적: LLM을 "탐지기"로만 사용. 판단 변경 여부 묻지 않음.
# 출력: 관련 가능성 있는 케이스 번호 목록만 (JSON 배열).
RULING_CROSS_CHECK_PROMPT = """\
아래 유권해석과 쟁점이 겹칠 가능성이 있는 케이스 번호를 찾아라.
...
## 지시사항
- 유권해석과 동일하거나 유사한 쟁점(조문, 특례 유형, 사실관계 패턴)이 겹치는 케이스 번호만 나열하라.
- 판정이 바뀌는지 여부는 판단하지 말라. 쟁점 겹침 여부만 판단하라.
- 확실하지 않으면 포함시키지 말라. 오탐보다 누락이 낫다.
- 반드시 아래 JSON 형식으로만 응답하라:
{{"related_indices": [0, 2, 5]}}
관련 케이스가 없으면: {{"related_indices": []}}"""
