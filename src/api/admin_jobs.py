...
ADMIN_COMMANDS: dict[str, AdminCommand] = {
    ...
    "regulatory_changes": AdminCommand(
        ...
        label="규제지역 변경 감지",
        description=(
            "전자관보·국토부·기재부·입법예고·뉴스 신호를 점검해 "
            "규제지역 변경 제안서를 생성합니다. 기준표는 data/area_designations/manual_table.json입니다."
        ),
        args=["src.ingestion.area_designation_pipeline"],
        path="data/area_designations/proposals",
        phase="collect",
        group="PDF/행정 데이터 수집",
        long=True,
    ),
    ...
    "rulings_incremental": AdminCommand(
        ...
        args=["scripts.ingestion.collect_and_embed_rulings"],
        path="data/rulings",
        phase="automation",
        group="수집+업로드 통합 자동화",
        paid=True,
    ...
    "law_changes_embed": AdminCommand(
        ...
        args=["scripts.ops.detect_law_changes", "--embed"],
