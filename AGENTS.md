...
> 이 섹션은 해커톤 마무리 시점(2026-05-28)의 알려진 한계를 명시한다.  
> 프로덕션 버전에서 해결해야 할 항목들이다.

### ① 평가셋 한계 — "AI가 만든 문제를 AI가 푸는" 구조

| 항목 | 현황 | 문제 |
|------|------|------|
| 골든셋 출처 | LLM 합성 142케이스 | 실제 세무사 케이스 아님 |
| 복합 특례 중첩 | 미포함 | 혼인합가+임대+수용 동시 등 |
| 경계값 설계자 | AI | 놓친 엣지케이스 있을 수 있음 |
| 정확도 의미 | 합성셋 기준 ~99% | 실전 정확도와 다름 |

### ② 커버리지 — 지금 못 하는 것들

| 항목 | 상태 | 비고 |
|------|------|------|
| 분양권 단기세율 | ⚠️ 불안정 | LLM이 일반과세로 오판 가능 |
| 법인 관련 거래 | ❌ 미지원 | 도메인 범위 외 |
| 부담부증여 세액 | ⚠️ 부분 | 시뮬레이션만, 판단 미완성 |
| 조정대상지역 자동조회 | ❌ 미구현 | 수동 입력 의존 |
| 복합 다주택 시나리오 | ⚠️ 부분 | 3주택 이상 검증 부족 |
| 기준시가(공시가격) 수집 | ❌ 미구현 | 외부 API 연동 없음 |

### ③ 검색(RAG) 품질

| 항목 | 현황 | 위험 |
|------|------|------|
| §121(비거주자) 등 일부 조문 | Pinecone 미수록 가능 | LLM 추측 → 사실관계부족 반환 |
| 2026년 이후 개정분 | 미확인 | 자동 감지는 있으나 검증 안 됨 |
| 유권해석 데이터 | 부분 수록 | 최신 예규 누락 가능 |
| BGE reranker 학습데이터 | 합성 50건 | 실전 판결문 기반 아님 |

### ④ 운영 안정성

| 항목 | 상태 |
|------|------|
| Pinecone API 장애 시 fallback | ❌ 없음 |
| LLM 비용 급증 방어 | ❌ rate limit 외 없음 |
| 개정세법 자동 반영 검증 | 🔧 로직 있으나 프로덕션 미검증 |
| 세무사 검토 없는 최종 판단 | ❌ 금지 — 보조 도구 수준 |

### ⑤ 프로덕션 전환 시 필수 작업

1. **실제 세무사 케이스 기반 골든셋** 재구축 (최소 500건)
2. **분양권·비거주자 단기세율** 결정론적 규칙 보강
3. **조정대상지역 실시간 API 연동** (국토부 or 부동산원)
4. **BGE reranker 파인튜닝** — 실전 판결문 데이터 50건 이상 확보 후
5. **Pinecone 조문 커버리지 감사** — 누락 조문 목록 작성 후 재인덱스
6. **세무사 검토 플로우** 의무화 (confidence < 0.9 → 자동 에스컬레이션)
