...
한국 양도소득세 판단을 위한 법령 RAG 엔진. JSON 사실관계 입력 → L1.5~L5 파이프라인 → TaxAnswer 반환.

> 아키텍처·스택·판단 원칙·디렉터리·환경 변수 상세 — [AGENTS.md](AGENTS.md) 참조.
...
## LLM 코딩 행동 지침 (Claude Code 전용)

이 지침은 일반적인 LLM 코딩 실수를 줄이기 위한 행동 규칙이다.  
빠른 속도보다 정확성을 우선한다. 사소한 작업은 판단해서 적용한다.

### 1. 코딩 전 먼저 생각한다

가정하지 않는다. 혼란을 숨기지 않는다. 트레이드오프를 드러낸다.

- 구현 전: 전제를 명시한다. 불확실하면 먼저 묻는다.
- 여러 해석이 가능하면 선택지를 제시한다 — 임의로 고르지 않는다.
- 더 단순한 접근이 있으면 말한다. 필요하면 반론을 제기한다.
- 불명확한 게 있으면 멈추고 무엇이 혼란스러운지 명시한 뒤 묻는다.

### 2. 단순성 우선

요청된 문제를 해결하는 최소 코드. 추측성 코드는 없다.
...
반드시 필요한 곳만 건드린다. 자신의 변경으로 생긴 것만 정리한다.

- 기존 코드를 "개선"하지 않는다 (관련 없는 주석·포맷 포함).
- 망가지지 않은 것은 리팩터하지 않는다.
- 기존 스타일을 그대로 맞춘다, 다르게 하고 싶어도.
- 관련 없는 dead code를 발견하면 언급만 한다 — 삭제하지 않는다.
- **모든 변경 라인은 사용자 요청으로 직접 추적 가능해야 한다.**

### 4. 목표 기반 실행

성공 기준을 정의한다. 검증될 때까지 루프한다.

- "검증 추가" → "유효하지 않은 입력에 대한 테스트를 먼저 작성하고, 통과시킨다"
- "버그 수정" → "재현 테스트를 작성하고, 통과시킨다"
- 다단계 작업은 간략한 계획을 명시한다: `[단계] → 검증: [확인 방법]`
